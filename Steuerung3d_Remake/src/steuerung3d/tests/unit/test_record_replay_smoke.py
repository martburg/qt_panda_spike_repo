from __future__ import annotations

from pathlib import Path

from steuerung3d.common.timebase import Timebase
from steuerung3d.core.engine import CoreEngine
from steuerung3d.core.intent_handler import apply_intent
from steuerung3d.core.intents import ArmLiveMode, EnableAxis, JogAxis, SetEstop
from steuerung3d.core.state import MachineState
from steuerung3d.protocol.recording import JsonlRecorder, LoggedTransport, JsonlReader
from steuerung3d.protocol.transport import InMemTransport


def test_record_and_replay(tmp_path: Path):
    log = tmp_path / "session.jsonl"

    # --- record ---
    raw = InMemTransport()
    rec = JsonlRecorder(log)
    tr = LoggedTransport(raw, rec)

    tb = Timebase(dt_s=0.01)
    st = MachineState()
    st.ensure_axis("X")

    eng = CoreEngine(
        timebase=tb,
        state=st,
        drain_intents=tr.drain_intents,
        handle_intent=apply_intent,
        on_step=lambda s, dt: _plant(s, dt),
        on_snapshot=tr.publish_telemetry,
        on_command_frame=rec.record_command_frame,
    )

    # inject some intents at known ticks
    tr.publish_intent(ArmLiveMode())          # stamped tick=0
    eng.step_once()                           # tick=1
    tr.publish_intent(EnableAxis(axis_id="X", enable=True))  # stamped tick=1
    tr.publish_intent(JogAxis(axis_id="X", vel=0.5))         # stamped tick=1

    for _ in range(20):
        eng.step_once()

    tr.publish_intent(SetEstop(estop=True))
    for _ in range(5):
        eng.step_once()

    final_pos_record = st.axes["X"].pos
    final_tick_record = st.tick

    # --- replay checks ---
    r = JsonlReader(log)

    snaps = list(r.iter_telemetry())
    frames = list(r.iter_command_frames())

    assert snaps, "expected telemetry in log"
    assert frames, "expected command frames in log"

    assert snaps[-1].tick == final_tick_record
    assert frames[-1].tick == final_tick_record
    assert abs(snaps[-1].axes["X"].pos - final_pos_record) < 1e-12

    # In this single-process setup we expect 1 CommandFrame per tick == 1 telemetry per tick
    assert len(frames) == len(snaps)
    assert [f.tick for f in frames] == [s.tick for s in snaps]


def _plant(state: MachineState, dt: float) -> None:
    ax = state.axes["X"]
    ax.pos += ax.vel * dt
